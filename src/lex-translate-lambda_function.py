...
def validate_phrase(phrase, source_lang, target_lang):
    if phrase is not None: 
        ...
        return build_validation_result(False,
                                       'phrase',
                                       'Initializing translate request from {} to {}. What is the phrase that you would like to translate?'.format(source_lang.capitalize(), target_lang.capitalize()))
...
def translatePhrase(intent_request):
    ...
    if source == 'DialogCodeHook':
        ...
        if phrase is not None:
            ...
            #Trigger SNS - translatorBot_DynamoDB_Storage
            arn = "arn:aws:sns:us-east-1:626480635631:translatorBot_DynamoDB_Storage"
            dict_msg = dict(phrase_text = phrase, source = source_lang, target =  target_lang, trans_text = translatedText)
            message = dict_msg
